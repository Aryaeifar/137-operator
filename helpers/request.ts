...
import type { RequestOptions, ApiOptions } from "../types/api";
...
class RequestClass {
  ...
  generateUrl(path: string, endpoint: string) {
    if(path.substring(0, 1) === '/' && (path.length === 1 || path.substring(0, 2) !== '//')) {
      return this.getBlogUrl() + path.replace(/\/*$/gm, '') + '/' + endpoint;
    }
